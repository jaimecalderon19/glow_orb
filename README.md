...
<div align="center">
  <img src="https://github.com/jaimecalderon19/glow_orb/screenshots/example.png" alt="Demo de la Cara de Asistente" width="250"/>
  <br>
...
- 🎨 Customizable gradient colors with pre-defined themes
- ⚡ Smooth rotating gradient animations
- 📏 Adjustable size
- 👁️ Simple eye design for personality
- 🎯 Tap interaction support
...
```dart
import 'package:animated_assistant_face/animated_assistant_face.dart';

AssistantFace(
  size: 200,
  onTap: () {
    print('Assistant face tapped!');
  },
)
```
...
AssistantFace(
  size: 200,
  gradientColors: AssistantFaceThemes.neon,
)
```

### Static (Non-animated) Face

```dart
AssistantFace(
  size: 150,
  isAnimated: false,
  gradientColors: AssistantFaceThemes.professional,
)
```

## Available Themes

- `AssistantFaceThemes.cool` - Cool blues and greens
- `AssistantFaceThemes.warm` - Warm reds and oranges  
- `AssistantFaceThemes.neon` - Bright neon colors
- `AssistantFaceThemes.professional` - Professional purple tones

## Parameters

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `size` | `double` | `200.0` | Size of the face widget |
| `gradientColors` | `List<Color>` | Cool theme | Colors for gradient background |
| `eyeColor` | `Color` | `Colors.white` | Color of the eyes |
| `animationDuration` | `Duration` | `Duration(seconds: 3)` | Animation cycle duration |
| `isAnimated` | `bool` | `true` | Whether gradient should animate |
| `onTap` | `VoidCallback?` | `null` | Callback when face is tapped |

## Example

Check out the `/example` folder for a complete example app showing different configurations and themes.
